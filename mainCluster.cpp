...
int main(int argc, char const *argv[]){
    ...
    switch (method){
        case __CLASIC_METHOD:
            io_files.writeClusterPoints(clustering->getClusters(), clustering->getClusteringTimes() ,clustering->getMedoids(), K, "Lloyds" , complete);
            //io_files.writeSilhouette(clustering->Silhouette(),K);
            break;
        case __LSH_METHOD:
            io_files.writeClusterPoints(clustering->getClusters(), clustering->getClusteringTimes() ,clustering->getMedoids(), K, "Range Search LSH", complete);
            //io_files.writeSilhouette(clustering->Silhouette(),K);
            break;
        default:
            io_files.writeClusterPoints(clustering->getClusters(), clustering->getClusteringTimes() ,clustering->getMedoids(), K, "Range Search Hypercube ", complete);
            //io_files.writeSilhouette(clustering->Silhouette(),K);
            break;
    }
